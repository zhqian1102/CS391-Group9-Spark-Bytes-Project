...
export default function ViewAttendeesPage() {
  ...
  const [error, setError] = useState(null);   // <-- New: visible error message
  ...
  async function loadPage() {
    try {
      ...
      if (!eventRes.ok) {
        ...
        setError(`Failed to load event: ${text}`);
        return;
      }
      ...
      // 3. Load attendees
      const attendeesRes = await fetch(`${API_URL}/api/events/${id}/attendees`, {
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
      });
  ...
  const exportToCSV = () => {
    ...
    const rows = attendees.map((a) => {
      const name = a.users?.full_name || "N/A";
      const email = a.users?.email || "N/A";
      const reg = new Date(a.created_at).toLocaleString();
  ...
  return (
    <div className="view-attendees-page">
      <NavigationBar />
                  ...
                  <table className="attendees-table">
                    ...
                    <tbody>
                      {attendees.map((att, i) => (
                        <tr key={att.id}>
                          <td>{i + 1}</td>
                          <td>{att.users?.full_name || "N/A"}</td>
                          <td>{att.users?.email || "N/A"}</td>
